...
IMAGE="${IMAGE:-ghcr.io/kodansha/bedrock:php8.5-fpm}"
...
setup_file() {
  ...
  local php_ver composer_ver wpcli_ver pie_ver
  ...
  pie_ver=$(docker exec "$container_id" pie --version 2>&1 | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
...
# ---------------------------------------------------------------------------
# FPM-specific: www-data user
# ---------------------------------------------------------------------------

@test "www-data UID is 101" {
  run dc id -u www-data
  assert_success
  assert_output "101"
}

@test "www-data GID is 101" {
  run dc id -g www-data
  assert_success
  assert_output "101"
}
